...
// Endpoint el cual retorna los valores para hacer reporte en gráfico de barras el cual muestra cuántas publicaciones ha
// realizado cada persona
app.get('/reportes', (req, res) => {
    // Objeto para almacenar todos los reportes individuales
    const resportes = {}; // Ej: { 'Rodolfo Morales': 3, 'Cristian Oxlaj': 1, 'Angel Barrera': 1 }

    // Recorremos el arreglo dataPosts
    dataPosts.forEach( post => {
        // Si el usuario ya existe en el objeto reportes, incrementar contador de publicaciones
        if (resportes.hasOwnProperty(post.name)) {
            resportes[post.name]++;
        } else{
            // Si el usuario no existe en los reportes, lo inicializamos con 1
            resportes[post.name] = 1;
        }
    });
    // Convertimos el objeto de reportes en un arreglo de objetos para dar una respuesta
    const reportesArray = Object.entries(resportes).map(([usuario,post]) => ({usuario, post}));
    res.json({response: reportesArray});
